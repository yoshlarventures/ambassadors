...
interface LeadReportsOverviewTableProps {
...
export function LeadReportsOverviewTable({
  ambassadors,
  reports,
  year,
  currentYear,
}: LeadReportsOverviewTableProps) {
  ...
  const handleYearChange = (direction: number) => {
    router.push(`/lead/reports/overview?year=${year + direction}`);
  };
            ...
            {ambassadors.length === 0 && (
              <TableRow>
                <TableCell colSpan={14} className="text-center text-muted-foreground py-8">
                  No ambassadors found in your region
                </TableCell>
              </TableRow>
            )}
          </TableBody>
