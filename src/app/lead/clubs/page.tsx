...
async function getRegionClubs(regionId: string) {
...
export default async function LeadClubsPage() {
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Manage clubs in your region</p>
        </div>
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            You need to be assigned to a region to manage clubs.
          </CardContent>
        </Card>
      </div>
    );
  ...
  const [clubs, region, ambassadors] = await Promise.all([
    getRegionClubs(user.region_id),
    getRegion(user.region_id),
    getRegionAmbassadors(user.region_id),
  ]);

  // Create a regions array with just the user's region for the dialog
              ...
              {clubs.map((club) => {
                ...
                return (
                  <TableRow key={club.id}>
                    <TableCell>
                      <Link href={`/lead/clubs/${club.id}`} className="block hover:underline">
